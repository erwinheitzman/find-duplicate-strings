...
describe('Output', () => {
	...
	it('should write with a custom file name', async () => {
		...
		expect(jest.mocked(writeFileSync).mock.calls[0][0]).toEqual(resolve(process.cwd(), 'foo-bar.json'));
	});
	...
	it('should update the output name if it already exists', async () => {
		...
		jest.mocked(existsSync).mockClear().mockReturnValueOnce(true).mockReturnValue(false);
		output.output();
	...
	it('should create two files when the findings do not exceed 99', async () => {
		...
		expect(jest.mocked(writeFileSync).mock.calls[1][0]).toEqual(resolve(process.cwd(), 'fds-output[0].json'));
		expect(jest.mocked(writeFileSync).mock.calls[2][0]).toEqual(resolve(process.cwd(), 'fds-output[1].json'));
	});
	...
	it('should create four files when the findings do not exceed 9999', async () => {
		...
		expect(jest.mocked(writeFileSync).mock.calls[1][0]).toEqual(resolve(process.cwd(), 'fds-output[0].json'));
		expect(jest.mocked(writeFileSync).mock.calls[2][0]).toEqual(resolve(process.cwd(), 'fds-output[1].json'));
		expect(jest.mocked(writeFileSync).mock.calls[3][0]).toEqual(resolve(process.cwd(), 'fds-output[2].json'));
		expect(jest.mocked(writeFileSync).mock.calls[4][0]).toEqual(resolve(process.cwd(), 'fds-output[3].json'));
	});
});
